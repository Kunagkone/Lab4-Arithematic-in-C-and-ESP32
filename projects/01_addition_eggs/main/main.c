...
void app_main(void)
{
    ...
    int apple_in_basket = 5;
    int apple_bought = 3;
    int total_apples = apple_in_basket + apple_bought;

    ESP_LOGI("ในตะกร้ามีแอปเปิ้ลอยู่แล้ว: %d ผล\n", apple_in_basket);
    ESP_LOGI("แม่ซื้อแอปเปิ้ลมาเพิ่ม: %d ผล\n", apple_bought);
    ESP_LOGI("แอปเปิ้ลทั้งหมดในตะกร้า: %d + %d = %d ผล\n", apple_in_basket, apple_bought, total_apples);
